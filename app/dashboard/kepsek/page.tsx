...
export default function KepsekDashboard() {
  ...
  const stats = [
    { label: "Total Siswa",      value: loading ? "..." : String(data.totalStudents),  icon: Users,        color: "text-[#2E7D32]",   bg: "bg-[#E8F5E9]"   },
    { label: "Rata-rata Nilai",  value: loading ? "..." : `${data.schoolAvgScore}%`,   icon: BookCheck,    color: "text-[#00897B]",   bg: "bg-[#E0F2F1]"   },
    { label: "Guru Aktif",       value: loading ? "..." : String(data.totalTeachers),  icon: TrendingUp,   color: "text-[#1976D2]",   bg: "bg-[#E3F2FD]"   },
    { label: "Siswa Tertinggal", value: loading ? "..." : "0",                         icon: AlertCircle,  color: "text-[#E53935]",   bg: "bg-[#FFEBEE]"   },
  ];

  function barColor(pct: number) {
    if (pct >= 85) return "bg-[#4CAF50]";
    if (pct >= 75) return "bg-[#00897B]";
    if (pct >= 60) return "bg-[#FF8F00]";
    return "bg-[#E53935]";
  }
