  ...
  useEffect(() => {
    async function loadDataMapel() {
      try {
        ...
        setAssignedSubjects(result.assignedSubjects);
        setAvailableAssignments(result.availableAssignments);
        setSelectedIds(result.assignedSubjects.map((sub: any) => sub.id));
        setSavedCount(result.assignedSubjects.length);
      } catch (err: any) {
