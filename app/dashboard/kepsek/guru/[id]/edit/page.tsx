...
export default function EditGuruPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function loadCurrentTeacher() {
      try {
        ...
        setFormData({
          ...
          isActive: data.teacher.user?.isActive ?? true
        });
      } catch (err: any) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    // Gabungkan data profil dengan password baru jika diisi
    ...
    try {
      ...
      setNewPassword(""); // Reset input password setelah sukses
